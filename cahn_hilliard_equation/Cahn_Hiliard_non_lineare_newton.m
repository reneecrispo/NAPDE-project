...
matrix_A_full = full(matrix_A);
matrix_M_full = full(matrix_M);
...
for n = 0:Nt-1
    ...
    matrix_T_full = full(matrix_T);

    fun = @(x) (matrix_M_full + dt * matrix_A_full - dt * matrix_T_full) * x - matrix_M_full * u_old;
    J = @(x) matrix_M_full + dt * matrix_A_full - dt * matrix_M_full + 2 * dt * matrix_M_full * x;
    niter = 1000;
    toll = 1e-3;

    [u, it] = newtonsys_cahn_hilliard(u_old, niter, toll, fun, J);
   
    % SALVATAGGIO DEI DATI IN PIU' FILE
    % NOME DELLA CARTELLA DA CAMBIARE OGNI VOLTA
    output_folder = 'results/results_d';
    ...
    output_file_name_n = sprintf('%s/results_d_%04d.vtk', output_folder, n);
