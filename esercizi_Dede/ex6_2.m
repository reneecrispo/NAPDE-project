...
f = @( x, y )( pi^2 * sin( pi/6 * x ) .* ( ( 1 / 36 + 2 ) * ( sin( pi * y ) ).^2 - 2 * ( cos( pi * y ) ).^2 ) );    % source term

drchlt_sides = [ 1 3 4 ];   % indexes for Dirichlet faces
nmnn_sides   = [ 2 ];   % indexes for Neuman faces

g = @( x, y, ind )( 0 + 0 * x );    % Dirichlet data (for different boundaries)
h = @( x, y, ind )( sqrt( 3 ) / 2 * pi / 6 * ( sin( pi * y ) ).^2 );    % Neuman data (for different boundaries)

% set exact solution, if available
%-----------------------------------
uex = @( x, y ) ( sin( pi / 6 * x ) .* sin( pi * y ).^2 );
graduex = @( x, y ) cat( 1, ...
                   reshape( pi / 6 * cos( pi / 6 * x ) .* ( sin( pi * y ) ).^2, [1, size(x)]), ...
                   reshape( 2 * pi * sin( pi / 6 * x ) .* sin( pi * y ) .* cos( pi * y ), [1, size(x)]) );
