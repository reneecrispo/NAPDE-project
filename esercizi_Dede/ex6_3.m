...
f = @( x, y )( - 1 / 3 * exp( x.^2 + y.^2 - 1 ) .* ( 12 + 4 * ( x.^2 + y.^2 ) - 8 * x.^ 2 .* y.^2 - 4 * ( x.^4 + y.^4 ) )  );    % source term

drchlt_sides = [ 1 2 ];   % indexes for Dirichlet faces
nmnn_sides   = [ 3 4 ];   % indexes for Neuman faces

g = @( x, y, ind )( 1 / 3 * ( 4 - x.^2 - y.^2 ) .* exp( x.^2 + y.^2 - 1 ) );    % Dirichlet data (for different boundaries)
h = @( x, y, ind )( 0 + 0 * x );    % Neuman data (for different boundaries)

% set exact solution, if available
%-----------------------------------
uex = @( x, y ) ( 1 / 3 * ( 4 - x.^2 - y.^2 ) .* exp( x.^2 + y.^2 - 1 ) );
graduex = @( x, y ) cat( 1, ...
                   reshape( 1 / 3 * ( 6 * x - 2 * x.^3 - 2 * x .* y.^2 ) .* exp( x.^2 + y.^2 - 1 ), [1, size(x)]), ...
                   reshape( 1 / 3 * ( 6 * y - 2 * y.^3 - 2 * x.^2 .* y ) .* exp( x.^2 + y.^2 - 1 ), [1, size(x)]) );
